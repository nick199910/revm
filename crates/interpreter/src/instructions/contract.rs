...
pub fn extcall_gas_calc<T, H: Host<T> + ?Sized>(
    interpreter: &mut Interpreter,
    host: &mut H,
    target: Address,
...
pub fn call<T, H: Host<T> + ?Sized, SPEC: Spec>(
    ...
    let Some(mut gas_limit) = calc_call_gas::<T, H, SPEC>(
        interpreter,
        is_cold,
        has_transfer,
        is_empty,
...
pub fn delegate_call<T, H: Host<T> + ?Sized, SPEC: Spec>(
    ...
    let Some(gas_limit) =
        calc_call_gas::<T, H, SPEC>(interpreter, is_cold, false, false, local_gas_limit)
    else {
        return;
    };

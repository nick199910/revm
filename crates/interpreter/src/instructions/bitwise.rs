...
pub fn iszero<T, H: Host<T> + ?Sized>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
pub fn bitand<T, H: Host<T> + ?Sized>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
pub fn bitxor<T, H: Host<T> + ?Sized>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
pub fn shl<T, H: Host<T> + ?Sized, SPEC: Spec>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
/// EIP-145: Bitwise shifting instructions in EVM
pub fn shr<T, H: Host<T> + ?Sized, SPEC: Spec>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
/// EIP-145: Bitwise shifting instructions in EVM
pub fn sar<T, H: Host<T> + ?Sized, SPEC: Spec>(
    interpreter: &mut Interpreter,
    _host: &mut H,
    _additional: &mut T,
) {
...
mod tests {
    ...
    fn test_shift_left() {
        ...
        for test in test_cases {
            ...
            shl::<u32, DummyHost, LatestSpec>(&mut interpreter, &mut host, &mut u32::MAX);
            pop!(interpreter, res);
    ...
    fn test_logical_shift_right() {
        ...
        for test in test_cases {
            ...
            push!(interpreter, test.value);
            push!(interpreter, test.shift);
            shr::<u32, DummyHost, LatestSpec>(&mut interpreter, &mut host, &mut u32::MAX);
            pop!(interpreter, res);
            assert_eq!(res, test.expected);
        }
    }
    ...
    fn test_arithmetic_shift_right() {
        ...
        for test in test_cases {
            ...
            push!(interpreter, test.value);
            push!(interpreter, test.shift);
            sar::<u32, DummyHost, LatestSpec>(&mut interpreter, &mut host, &mut u32::MAX);
            pop!(interpreter, res);
            assert_eq!(res, test.expected);
        }
    }
